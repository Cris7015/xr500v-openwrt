...
The board exposes four pass-through UART pads in a **vertical column between the SoC and the green GPON connector**, with no silkscreen labels. Serial parameters are **115200 8N1**. A 3.3 V USB-UART adapter (e.g. CP2102 → `/dev/ttyUSB0`) is used; `picocom --logfile` gives clean boot-log capture. The four pads are GND, VCC, TX and RX; GND/VCC are identified with a multimeter and TX/RX by elimination (do not drive VCC from the adapter).

...
/*
 * One-shot EN7570 receiver/APD A2 one-LSB observer for the TP-Link XR500v.
 *
...
static bool arm_en7570_rx_apd_a2_lsb;
module_param(arm_en7570_rx_apd_a2_lsb, bool, 0444);
MODULE_PARM_DESC(arm_en7570_rx_apd_a2_lsb,
		 "Arm the one-shot, non-transactional XR500v EN7570 RX/APD A2 LSB observer");

static atomic_t rx_apd_lsb_sequence_claimed = ATOMIC_INIT(0);
...
struct xr500v_rx_apd_lsb_observer {
	struct device *dev;
...
static u32 xpon_read(struct xr500v_rx_apd_lsb_observer *observer, u32 reg)
{
	return ioread32(observer->xpon_base + reg);
}

static int en7570_read(struct xr500v_rx_apd_lsb_observer *observer, u16 reg,
...
static int en7570_read4(struct xr500v_rx_apd_lsb_observer *observer, u16 reg,
			u8 value[4])
{
...
static int gpio_xpon_gate(struct xr500v_rx_apd_lsb_observer *observer)
{
	int direction;
	int logical;
	int raw;
...
static int fast_tx_gate(struct xr500v_rx_apd_lsb_observer *observer)
{
	u8 value[4];
	int ret;
...
static void full_snapshot_capture(struct xr500v_rx_apd_lsb_observer *observer,
...
static int full_snapshot_verify(struct xr500v_rx_apd_lsb_observer *observer,
...
static void capture_tx_guards(struct xr500v_rx_apd_lsb_observer *observer,
...
static void capture_critical(struct xr500v_rx_apd_lsb_observer *observer,
 ...
 */
static void apd_snapshot_capture(struct xr500v_rx_apd_lsb_observer *observer,
				 struct apd_snapshot *snapshot,
				 bool post_attempt)
{
	memset(snapshot, 0, sizeof(*snapshot));
...
static int apd_snapshot_verify(struct xr500v_rx_apd_lsb_observer *observer,
...
static int en7570_fixed_write_once(struct xr500v_rx_apd_lsb_observer *observer,
				   unsigned int step)
{
...
static int en7570_read_expect(struct xr500v_rx_apd_lsb_observer *observer,
...
static int run_reset_rssi_gain_los(struct xr500v_rx_apd_lsb_observer *observer)
{
...
static int write_en7570_rx_polarity(struct xr500v_rx_apd_lsb_observer *observer)
{
	int ret;
...
static int rx_sample_read4(struct xr500v_rx_apd_lsb_observer *observer,
...
static void rx_sample_capture(struct xr500v_rx_apd_lsb_observer *observer,
...
static int run_rx_samples(struct xr500v_rx_apd_lsb_observer *observer)
{
	unsigned int i;
	int ret;
...
static int verify_factory_hash(struct xr500v_rx_apd_lsb_observer *observer)
{
...
static int rx_apd_lsb_status_show(struct seq_file *s, void *unused)
{
	struct xr500v_rx_apd_lsb_observer *observer = s->private;
	int i;

	seq_puts(s, "operation:             fixed OEM EN7570 reset/RSSI/gain/LOS + RX polarity + APD A2 one-LSB oracle\n");
...
DEFINE_SHOW_ATTRIBUTE(rx_apd_lsb_status);

static void rx_apd_lsb_results_init(struct xr500v_rx_apd_lsb_observer *observer)
{
	int i;
...
static int rx_apd_lsb_run(struct xr500v_rx_apd_lsb_observer *observer)
{
	int ret;
	int i;

	/* Globally consume the one sequence allowed for this module load. */
	if (atomic_cmpxchg(&rx_apd_lsb_sequence_claimed, 0, 1))
		return -EBUSY;
...
static int xr500v_rx_apd_lsb_observer_probe(struct platform_device *pdev)
{
	struct xr500v_rx_apd_lsb_observer *observer;
	...
	if (observer->cold_verify_result) {
		...
		dev_err(&pdev->dev, "exact RX/APD A2 LSB cold preflight failed: %d\n",
			ret);
		goto out_unlock;
	}

	ret = rx_apd_lsb_run(observer);
	if (ret) {
		observer->sequence_result = ret;
...
out_unlock:
	...
	observer->debugfs_dir =
		debugfs_create_dir("xr500v-en7570-rx-apd-a2-lsb-observer", NULL);
	if (IS_ERR_OR_NULL(observer->debugfs_dir)) {
		dev_err(&pdev->dev,
			"debugfs directory unavailable after RX/APD A2 LSB attempt: %d\n",
		...
		observer->debugfs_status =
			debugfs_create_file("status", 0444, observer->debugfs_dir,
					    observer, &rx_apd_lsb_status_fops);
		if (IS_ERR_OR_NULL(observer->debugfs_status))
			dev_err(&pdev->dev,
				"debugfs status unavailable after RX/APD A2 LSB attempt: %d\n",
				PTR_ERR_OR_ZERO(observer->debugfs_status));
	}

	platform_set_drvdata(pdev, observer);
	dev_warn(&pdev->dev,
		 "EN7570 RX/APD A2 LSB result %d after %u I2C and %u MMIO write attempt(s), %u sample(s); TX_DISABLE retained; physical power removal required\n",
		 observer->sequence_result, observer->i2c_write_attempts,
		 observer->mmio_write_attempts, observer->samples_taken);
	return 0;
}

static void xr500v_rx_apd_lsb_observer_remove(struct platform_device *pdev)
{
	struct xr500v_rx_apd_lsb_observer *observer = platform_get_drvdata(pdev);

	debugfs_remove_recursive(observer->debugfs_dir);
	dev_emerg(&pdev->dev,
		  "RX/APD A2 LSB observer removal has no rollback; remove physical power now\n");
}

static const struct of_device_id xr500v_rx_apd_lsb_observer_of_match[] = {
	{ .compatible = "econet,en751221-en7570-rx-apd-a2-lsb-experimental" },
	{ }
};
MODULE_DEVICE_TABLE(of, xr500v_rx_apd_lsb_observer_of_match);

static struct platform_driver xr500v_rx_apd_lsb_observer_driver = {
	.probe = xr500v_rx_apd_lsb_observer_probe,
	.remove = xr500v_rx_apd_lsb_observer_remove,
	.driver = {
		.name = "xr500v-en7570-rx-apd-a2-lsb-observer",
		.of_match_table = xr500v_rx_apd_lsb_observer_of_match,
		.suppress_bind_attrs = true,
	},
};
module_platform_driver(xr500v_rx_apd_lsb_observer_driver);

MODULE_DESCRIPTION("Guarded XR500v EN7570 fixed RX/APD A2 one-LSB observer");
